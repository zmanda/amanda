    ...
    char *	date)
{
	...
	g_free(cmd);
	cmd = g_strdup_printf("OISD %s", qdisk_path);
	amfree(qdisk_path);
	if (exchange(cmd) == -1)
	    exit(1);
	if (server_happy())
    ...
    char *	mtpt)
{
    ...
    cmd = g_strdup_printf("DISK %s", qdsk);
    amfree(qdsk);
    if (converse(cmd) == -1)
	exit(1);
    amfree(cmd);
    ...
    char *	amdevice)
{
	...
	cmd = g_strdup_printf("LISTDISK %s", qamdevice);
	amfree(uqamdevice);
	amfree(qamdevice);
	if (converse(cmd) == -1)
	    exit(1);
    ...
    int		verbose)
{
	...
	g_printf("\"%s\" is not a valid regular expression: ", uq_orig_regex);
	amfree(uqregex);
    ...
    /* convert path (assumed in cwd) to one on disk */
    if (strcmp(disk_path, "/") == 0)
        path_on_disk = g_strdup_printf("/%s", uqregex);
    else {
        char *clean_disk_path = clean_regex(disk_path, 0);
    ...
    int		verbose)
{
	...
	g_printf("Too many directories matching '%s'\n", default_dir);
	result = 0;
    }
    amfree(dir);
    return result;
    ...
    int		verbose)
{
    ...
    cmd = g_strdup_printf("OISD %s", qnew_dir);
    amfree(qnew_dir);
    if (exchange(cmd) == -1) {
	exit(1);
	/*NOTREACHED*/
    ...
    char *	tape)
{
    ...
    g_printf("NOTE: 'settape' is deprecated; use setdevice instead.\n");
