 ...
 * on one or more holdings, without any caching.  This destination supports both
...
typedef struct XferDestHolding {
    ...
    volatile char * volatile filename;
    volatile char * volatile tmp_filename;
    volatile char * volatile first_filename;
    volatile char * volatile new_filename;
    volatile dumpfile_t * volatile chunk_header;
    volatile int fd;
    volatile guint64 use_bytes;
    volatile guint64 data_bytes_written; /* bytes written in the current call */
    volatile guint64 header_bytes_written;
    volatile guint64 chunk_offset;       /* bytes written to the current */
...
/* local functions */
static void close_chunk(XferDestHolding *xdt, char *cont_filename);
static ssize_t write_header(XferDestHolding *xdt, int fd);
...
#define DBG(LEVEL, ...) if (debug_chunker >= LEVEL) { _xdt_dbg(__VA_ARGS__); }
static void
    ...
    XferDestHolding *self)
{
	...
	if (to_write == 0) {
	    ...
	    break;
	} else if (self->chunk_status == CHUNK_EOC) {
	    break;
	}
	...
	self->data_bytes_written += to_write;
	self->use_bytes -= to_write;
	holding_thread_consume_block(self, to_write);
    ...
    gpointer data)
{
	...
	if (self->filename == NULL ||
	    strcmp((char *)self->filename, (char *)self->new_filename) != 0) {
	    char    *tmp_filename;
	    char    *pc;
	    int      fd;
	    ssize_t  write_header_size;

	    if (self->use_bytes < HOLDING_BLOCK_BYTES) {
		self->chunk_status = CHUNK_NO_ROOM;
		goto no_room;
	    }
	    ...
	    *pc = '/';
	    ...
	    self->chunk_header->cont_filename[0] = '\0';
	    if (write_header_size != HOLDING_BLOCK_BYTES) {
		self->chunk_header->cont_filename[0] = '\0';
		self->chunk_status = CHUNK_NO_ROOM;
	    ...
	    self->use_bytes -= HOLDING_BLOCK_BYTES;

	    /* rewrite old_header */
	    if (self->filename &&
		strcmp((char *)self->filename, (char *)self->new_filename) != 0) {
		close_chunk(self, (char *)self->new_filename);
	    }
	    self->filename = self->new_filename;
	    self->tmp_filename = tmp_filename;
	    self->new_filename = NULL;
	    self->fd = fd;
	    self->header_bytes_written = HOLDING_BLOCK_BYTES;
	    self->chunk_offset = HOLDING_BLOCK_BYTES;
	}
...
start_chunk_impl(
    ...
    guint64 use_bytes)
{
    XferDestHolding *self = XFER_DEST_HOLDING(xdt);
    ...
    char *cont_filename)
{
    ...
    if (strcmp((char *)self->filename, (char *)self->first_filename) == 0) {
	self->chunk_header->type = F_DUMPFILE;
    } else {
	self->chunk_header->type = F_CONT_DUMPFILE;
    }
    if (cont_filename) {
	strncpy((char *)self->chunk_header->cont_filename, cont_filename, sizeof(self->chunk_header->cont_filename));
    ...
    close(self->fd);
    self->fd = -1;
    g_free((char *)self->tmp_filename);
    self->tmp_filename = NULL;
    g_free((char *)self->filename);
    self->filename = NULL;
}

static guint64
get_chunk_bytes_written_impl(
    XferDestHolding *xdtself)
{
    XferDestHolding *self = XFER_DEST_HOLDING(xdtself);
    ...
    XferDestHoldingClass * selfc)
{
    ...
    XferDestHoldingClass *xdt_klass = XFER_DEST_HOLDING_CLASS(selfc);
    ...
    xdt_klass->start_chunk = start_chunk_impl;
    xdt_klass->finish_chunk = finish_chunk_impl;
    xdt_klass->get_chunk_bytes_written = get_chunk_bytes_written_impl;
    ...
    int fd)
{
    ...
    self->chunk_header->blocksize = HOLDING_BLOCK_BYTES;
    if (debug_chunker > 1)
        dump_dumpfile_t((dumpfile_t *)self->chunk_header);
    buffer = build_header((dumpfile_t *)self->chunk_header, NULL, HOLDING_BLOCK_BYTES);
    if (!buffer) /* this shouldn't happen */
        error(_("header does not fit in %zd bytes"), (size_t)HOLDING_BLOCK_BYTES);

    written = db_full_write(fd, buffer, HOLDING_BLOCK_BYTES);
    amfree(buffer);
    if(written == HOLDING_BLOCK_BYTES) return HOLDING_BLOCK_BYTES;
