...
usage(void)
{
    g_printf("Usage: amcheck [-am] [-w] [-sclt] [-M <address>] [-o configoption]* <conf> [host [disk]* ]*\n");
    exit(1);
    /*NOTREACHED*/
}
    ...
    char **	argv)
{
	...
	if (alwaysmail == 1) {
	    g_printf("You can't use -a because a mailer is not defined\n");
	} else {
	    g_printf("You can't use -m because a mailer is not defined\n");
	}
	exit(1);
    }
    if(mailout && !mailto &&
	...
	g_printf("To receive dump results by email configure the ""\"mailto\" parameter in amanda.conf\n");
        if(alwaysmail)
 		g_printf("When using -a option please specify -Maddress also\n\n");
	else
 		g_printf("Use -Maddress instead of -m\n\n"); 
	exit(1);
    }
    if(mailout && !mailto)
    {
	...
	if((pid = wait(&retstat)) == -1) {
	    ...
	    wait_msg = g_strdup_printf("parent: reaped bogus pid %ld\n", (long)pid);
	...
	if(mailto) {
	    ...
	    if (!a) {
 		g_printf("Invalid mailto address '%s'\n\n", getconf_str(CNF_MAILTO));
		exit(1);
	    }
	}
    ...
    char *tapefile)
{
	...
	if (!S_ISREG(statbuf.st_mode)) {
	    ...
	    g_fprintf(outf, "ERROR: tapelist %s: should be a regular file.\n",
		    quoted);
	    tapebad = 1;
	    amfree(quoted);
	} else if (access(tapefile, F_OK) != 0) {
	    quoted = quote_string(tapefile);
	    g_fprintf(outf, "ERROR: can't access tapelist %s\n", quoted);
	    tapebad = 1;
	    amfree(quoted);
	} else if (access(tapefile, W_OK) != 0) {
	    quoted = quote_string(tapefile);
	    g_fprintf(outf, "ERROR: tapelist %s: not writable\n", quoted);
	    tapebad = 1;
	    amfree(quoted);
	}
    }
    ...
    uid_t	dumpuid)
{
	...
	g_fprintf(outf, "ERROR: program %s: does not exist\n",
		quoted);
	pgmbad = 1;
    } else if (!S_ISREG(statbuf.st_mode)) {
	g_fprintf(outf, "ERROR: program %s: not a file\n",
		quoted);
	pgmbad = 1;
    } else if (access(pgm, X_OK) == -1) {
	g_fprintf(outf, "ERROR: program %s: not executable\n",
		quoted);
	pgmbad = 1;
	...
	g_fprintf(outf, "ERROR: program %s: not setuid-root\n",
		quoted);
	pgmbad = 1;
#else
...
static gboolean test_tape_status(FILE * outf) {
	...
	g_fprintf(outf, "amcheck-device terminated with signal %d",
    ...
    int		do_tapechk)
{
    ...
    g_fprintf(outf, "Amanda Tape Server Host Check\n");
    g_fprintf(outf, "-----------------------------\n");
	...
	if(strcmp(lbl_templ, "") != 0) {
		...
		g_fprintf(outf,
			"ERROR: cannot read label template (lbl-templ) file %s: %s. Check permissions\n",
			lbl_templ,
			strerror(errno));
		confbad = 1;
	    }
#if !defined(HAVE_LPR_CMD)
	    g_fprintf(outf, "ERROR: lbl-templ set but no LPR command defined. You should reconfigure amanda\n       and make sure it finds a lpr or lp command.\n");
	    confbad = 1;
#endif
	}
	...
	if (getconf_int(CNF_FLUSH_THRESHOLD_SCHEDULED) <
	    ...
	    g_fprintf(outf, "WARNING: flush-threshold-dumped (%d) must be less than or equal to flush-threshold-scheduled (%d).\n",
		      getconf_int(CNF_FLUSH_THRESHOLD_DUMPED),
	...
	if (getconf_int(CNF_FLUSH_THRESHOLD_SCHEDULED) <
	    ...
	    g_fprintf(outf, "WARNING: taperflush (%d) must be less than or equal to flush-threshold-scheduled (%d).\n",
		      getconf_int(CNF_TAPERFLUSH),
		      getconf_int(CNF_FLUSH_THRESHOLD_SCHEDULED));
	}
	...
	if (getconf_int(CNF_TAPERFLUSH) > 0 &&
	    ...
	    g_fprintf(outf, "WARNING: autoflush must be set to 'yes' or 'all' if taperflush (%d) is greater that 0.\n",
		      getconf_int(CNF_TAPERFLUSH));
	}

	/* Double-check that 'localhost' resolves properly */
	if ((res = resolve_hostname("localhost", 0, NULL, NULL) != 0)) {
	    g_fprintf(outf, "ERROR: Cannot resolve `localhost': %s\n", gai_strerror(res));
	    confbad = 1;
	}
	...
	if(access(amlibexecdir, X_OK) == -1) {
	    ...
	    g_fprintf(outf, "ERROR: Directory %s containing Amanda tools is not accessible\n.",
		    quoted);
	    g_fprintf(outf, "Check permissions\n");
	    pgmbad = 1;
	    amfree(quoted);
	} else {
	...
	if(access(COMPRESS_PATH, X_OK) == -1) {
	    ...
	    g_fprintf(outf, "WARNING: %s is not executable, server-compression ""and indexing will not work. \n",quoted);
	    g_fprintf(outf, "Check permissions\n");
	    amfree(quoted);
	}
    }
	...
	if(access(tape_dir, W_OK) == -1) {
	    ...
	    g_fprintf(outf, "ERROR: tapelist dir %s: not writable.\nCheck permissions\n", 
		    quoted);
	    tapebad = 1;
	    amfree(quoted);
	}
	    ...
	    if (errno != ENOENT) {
		quoted = quote_string(tape_dir);
		g_fprintf(outf, "ERROR: tapelist %s (%s), ""you must create an empty file.\n",
			quoted, strerror(errno));
		tapebad = 1;
		amfree(quoted);
	    } else {
		g_fprintf(outf, "NOTE: tapelist will be created on the next run.\n");
	    }
	} else {
	    tapebad |= check_tapefile(outf, tapefile);
	    if (tapebad == 0 && read_tapelist(tapefile)) {
		quoted = quote_string(tapefile);
		g_fprintf(outf, "ERROR: tapelist %s: parse error\n", quoted);
		tapebad = 1;
		amfree(quoted);
	    }
	...
	if(access(holdfile, F_OK) != -1) {
	    ...
	    g_fprintf(outf, "WARNING: hold file %s exists.", holdfile);
	    g_fprintf(outf, "Amdump will sleep as long as this file exists.\n");
	    g_fprintf(outf, "You might want to delete the existing hold file\n");
	    amfree(quoted);
	}
	amfree(tapefile);
	amfree(tape_dir);
	...
	if (tapename == NULL) {
		...
		g_fprintf(outf, "WARNING:Parameter \"tapedev\" or \"tpchanger\" not specified in amanda.conf.\n");
		testtape = 0;
		do_tapechk = 0;
	    }
	}
	...
	for (il = getconf_identlist(CNF_HOLDINGDISK);
		...
		g_fprintf(outf, "ERROR: holding dir %s (%s), ""you must create a directory.\n",
			quoted, strerror(errno));
		disklow = 1;
		amfree(quoted);
		continue;
		...
		g_fprintf(outf, "ERROR: holding disk %s: not writable: %s.\n",
			quoted, strerror(errno));
		g_fprintf(outf, "Check permissions\n");
		disklow = 1;
	    }
	    else if(access(holdingdisk_get_diskdir(hdp), X_OK) == -1) {
		g_fprintf(outf, "ERROR: holding disk %s: not searcheable: %s.\n",
			quoted, strerror(errno));
		g_fprintf(outf, "Check permissions of ancestors of %s\n", quoted);
		disklow = 1;
	    }
	    else if(holdingdisk_get_disksize(hdp) > (off_t)0) {
		if(kb_avail == 0) {
		    g_fprintf(outf,
			    "WARNING: holding disk %s: ""no space available (%lld %sB requested)\n", quoted,
		    ...
		    g_fprintf(outf,
			    "WARNING: holding disk %s: ""only %lld %sB available (%lld %sB requested)\n", quoted,
		    ...
		    g_fprintf(outf,
			    "Holding disk %s: %lld %sB disk space available,"" using %lld %sB as requested\n",
			    quoted,
			    (long long)(kb_avail / (off_t)unitdivisor),
			    displayunit,
			    (long long)(holdingdisk_get_disksize(hdp)/(off_t)unitdivisor),
		...
		if(kb_avail < -holdingdisk_get_disksize(hdp)) {
		    g_fprintf(outf,
			    "WARNING: holding disk %s: ""only %lld %sB free, using nothing\n",
			    quoted, (long long)(kb_avail / (off_t)unitdivisor),
			    displayunit);
	            g_fprintf(outf, "WARNING: Not enough free space specified in amanda.conf\n");
		    disklow = 1;
		}
		else {
		    g_fprintf(outf,
	...
	if(stat(conf_logdir, &statbuf) == -1) {
	    g_fprintf(outf, "ERROR: logdir %s (%s), you must create directory.\n",
		    quoted, strerror(errno));
	    disklow = 1;
	}
	else if(access(conf_logdir, W_OK) == -1) {
	    g_fprintf(outf, "ERROR: log dir %s: not writable\n", quoted);
	    logbad = 1;
	}
	amfree(quoted);
	...
	if(logbad == 0 && access(logfile, F_OK) == 0) {
		...
		g_fprintf(outf, "ERROR: log file %s: not writable\n", quoted);
		amfree(quoted);
	    }
	}
	...
	if (logbad == 0 && stat(olddir,&stat_old) == 0) { /* oldlog exist */
		...
		g_fprintf(outf, "ERROR: oldlog directory %s is not a directory\n",
			quoted);
		g_fprintf(outf, "Remove the entry and create a new directory\n");
		logbad = 1;
	    }
	    if(logbad == 0 && access(olddir, W_OK) == -1) {
		g_fprintf(outf, "ERROR: oldlog dir %s: not writable\n", quoted);
		g_fprintf(outf, "Check permissions\n");
		logbad = 1;
	    }
	}
	...
	g_fprintf(outf, "WARNING: skipping tape test because amdump or amflush seem to be running\n");
	g_fprintf(outf, "WARNING: if they are not, you must run amcleanup\n");
    } else if (logbad == 2) {
	g_fprintf(outf, "NOTE: amdump or amflush seem to be running\n");
	g_fprintf(outf, "NOTE: if they are not, you must run amcleanup\n");

	/* we skipped the tape checks, but this is just a NOTE and
	 * should not result in a nonzero exit status, so reset logbad to 0 */
	logbad = 0;
    } else {
	g_fprintf(outf, "NOTE: skipping tape checks\n");
    }
	...
	if(conf_tapecycle <= conf_runspercycle) {
		g_fprintf(outf, "WARNING: tapecycle (%d) <= runspercycle (%d).\n",
	...
	if(stat(conf_infofile, &statbuf) == -1) {
	    if (errno == ENOENT) {
		g_fprintf(outf, "NOTE: conf info dir %s does not exist\n",
			quoted);
		g_fprintf(outf, "NOTE: it will be created on the next run.\n");
	    } else {
		g_fprintf(outf, "ERROR: conf info dir %s (%s)\n",
			quoted, strerror(errno));
		infobad = 1;
	    }	
	    amfree(conf_infofile);
	} else if (!S_ISDIR(statbuf.st_mode)) {
	    g_fprintf(outf, "ERROR: info dir %s: not a directory\n", quoted);
	    g_fprintf(outf, "Remove the entry and create a new directory\n");
	    amfree(conf_infofile);
	    infobad = 1;
	} else if (access(conf_infofile, W_OK) == -1) {
	    g_fprintf(outf, "ERROR: info dir %s: not writable\n", quoted);
	    g_fprintf(outf, "Check permissions\n");
	    amfree(conf_infofile);
	    infobad = 1;
	} else {
	...
	while(!empty(origq)) {
		...
		if(stat(hostinfodir, &statbuf) == -1) {
		    if (errno == ENOENT) {
			g_fprintf(outf, "NOTE: host info dir %s does not exist\n",
				quoted);
			g_fprintf(outf,
				"NOTE: it will be created on the next run.\n");
		    } else {
			g_fprintf(outf, "ERROR: host info dir %s (%s)\n",
				quoted, strerror(errno));
			infobad = 1;
		    }	
		    amfree(hostinfodir);
		...
		if(hostinfodir) {
			...
			if (errno == ENOENT) {
			    g_fprintf(outf, "NOTE: info dir %s does not exist\n",
				quoted);
			    g_fprintf(outf,
				"NOTE: it will be created on the next run.\n");
			} else {
			    g_fprintf(outf, "ERROR: info dir %s (%s)\n",
				    quoted, strerror(errno));
			    infobad = 1;
			}	
		    } else if (!S_ISDIR(statbuf.st_mode)) {
			...
			if (errno == ENOENT) {
			    g_fprintf(outf, "NOTE: info file %s does not exist\n",
				    quotedif);
			    g_fprintf(outf, "NOTE: it will be created on the next run.\n");
			} else {
			    g_fprintf(outf, "ERROR: info dir %s (%s)\n",
			...
			g_fprintf(outf, "ERROR: info file %s: not a file\n",
				quotedif);
			g_fprintf(outf, "Remove the entry and create a new file\n");
			infobad = 1;
		    } else if (access(infofile, R_OK) == -1) {
			g_fprintf(outf, "ERROR: info file %s: not readable\n",
				quotedif);
			infobad = 1;
		    }
		    amfree(quotedif);
		...
		if(dp->index) {
			...
			if(stat(conf_indexdir, &statbuf) == -1) {
			    if (errno == ENOENT) {
				g_fprintf(outf, "NOTE: index dir %s does not exist\n",
				        quoted);
				g_fprintf(outf, "NOTE: it will be created on the next run.\n");
			    } else {
				g_fprintf(outf, "ERROR: index dir %s (%s)\n",
					quoted, strerror(errno));
				indexbad = 1;
			    }	
			    amfree(conf_indexdir);
			} else if (!S_ISDIR(statbuf.st_mode)) {
			    g_fprintf(outf, "ERROR: index dir %s: not a directory\n",
				    quoted);
			    g_fprintf(outf, "Remove the entry and create a new directory\n");
			    amfree(conf_indexdir);
			    indexbad = 1;
			} else if (access(conf_indexdir, W_OK) == -1) {
			    g_fprintf(outf, "ERROR: index dir %s: not writable\n",
				    quoted);
			    amfree(conf_indexdir);
			    indexbad = 1;
			} else {
			...
			if(hostindexdir) {
				...
				g_fprintf(outf, "ERROR: index dir %s: is not writable\n",
					quoted);
				indexbad = 1;
			    }
			    amfree(quoted);
		...
		if ( dp->encrypt == ENCRYPT_SERV_CUST ) {
		  if ( dp->srv_encrypt[0] == '\0' ) {
		    g_fprintf(outf, "ERROR: server encryption program not specified\n");
		    g_fprintf(outf, "Specify \"server-custom-encrypt\" in the dumptype\n");
		    pgmbad = 1;
		  }
		  else if(access(dp->srv_encrypt, X_OK) == -1) {
		    g_fprintf(outf, "ERROR: %s is not executable, server encryption will not work\n",
			    dp->srv_encrypt );
		   g_fprintf(outf, "Check file type\n");
		    pgmbad = 1;
		  }
		}
		if ( dp->compress == COMP_SERVER_CUST ) {
		  if ( dp->srvcompprog[0] == '\0' ) {
		    g_fprintf(outf, "ERROR: server custom compression program ""not specified\n");
		    g_fprintf(outf, "Specify \"server-custom-compress\" in ""the dumptype\n");
		    pgmbad = 1;
		  }
		  else if(access(dp->srvcompprog, X_OK) == -1) {
		    quoted = quote_string(dp->srvcompprog);

		    g_fprintf(outf, "ERROR: %s is not executable, server custom ""compression will not work\n",
			    quoted);
		    amfree(quoted);
		   g_fprintf(outf, "Check file type\n");
		    pgmbad = 1;
    ...
    g_fprintf(outf, "Server check took %s seconds\n", walltime_str(curclock()));
    ...
    am_host_t *hostp)
{
	...
	if(!am_has_feature(hostp->features, fe_selfcheck_req) &&
	   !am_has_feature(hostp->features, fe_selfcheck_req_device)) {
	    g_fprintf(outf,
		    ...
		    hostp->hostname);
	    g_fprintf(outf, "Client might be of a very old version\n");
	}
	...
	for(dp = hostp->disks; dp != NULL; dp = dp->hostnext) {
		...
		if(!am_has_feature(hostp->features, fe_interface_quoted_text)) {
		    ...
		    g_fprintf(outf, "You must upgrade amanda on the client to ""specify a quoted text/device in the disklist, ""or don't use quoted text for the device.\n");
		}
	    }
		...
		if(!am_has_feature(hostp->features, fe_selfcheck_req_device)) {
		    ...
		    g_fprintf(outf, "You must upgrade amanda on the client to ""specify a diskdevice in the disklist ""or don't specify a diskdevice in the disklist.\n");	
		}
		...
		if(strcmp(dp->program, "DUMP") == 0 &&
		   !am_has_feature(hostp->features, fe_program_dump)) {
		    g_fprintf(outf, "ERROR: %s:%s does not support DUMP.\n",
			    hostp->hostname, qname);
		    g_fprintf(outf, "You must upgrade amanda on the client to use DUMP ""or you can use another program.\n");	
		}
		if(strcmp(dp->program, "GNUTAR") == 0 &&
		   !am_has_feature(hostp->features, fe_program_gnutar)) {
		    g_fprintf(outf, "ERROR: %s:%s does not support GNUTAR.\n",
			    hostp->hostname, qname);
		    g_fprintf(outf, "You must upgrade amanda on the client to use GNUTAR ""or you can use another program.\n");	
		}
		...
		if(estimate == ES_CALCSIZE &&
		   !am_has_feature(hostp->features, fe_calcsize_estimate)) {
		    g_fprintf(outf, "ERROR: %s:%s does not support CALCSIZE for ""estimate, using CLIENT.\n",
			    hostp->hostname, qname);
		    g_fprintf(outf, "You must upgrade amanda on the client to use ""CALCSIZE for estimate or don't use CALCSIZE for estimate.\n");
		    estimate = ES_CLIENT;
		}
		if(estimate == ES_CALCSIZE &&
		...
		if(dp->compress == COMP_CUST &&
		  ...
		  g_fprintf(outf,
			  ...
			  hostp->hostname);
		    g_fprintf(outf, "You must upgrade amanda on the client to ""use custom compression\n");
		    g_fprintf(outf, "Otherwise you can use the default client ""compression program.\n");
		}
		if(dp->encrypt == ENCRYPT_CUST ) {
		  if ( !am_has_feature(hostp->features, fe_options_encrypt_cust)) {
		    g_fprintf(outf,
			    ...
			    hostp->hostname);
		    g_fprintf(outf, "You must upgrade amanda on the client to use encryption program.\n");
		    remote_errors++;
		...
		if (!am_has_feature(hostp->features, fe_program_application_api) ||
		    ...
		    g_fprintf(outf, "ERROR: %s:%s does not support APPLICATION-API.\n",
			    hostp->hostname, qname);
		    g_fprintf(outf, "Dumptype configuration is not GNUTAR or DUMP."" It is case sensitive\n");
		    remote_errors++;
		    l = g_strdup("");
		} else {
		    l = g_strjoin(NULL, "<dle>\n"
    ...
    if (secdrv == NULL) {
	fprintf(stderr, "Could not find security driver \"%s\" for host \"%s\". auth for this dle is invalid\n",
    ...
    int		fd)
{
    ...
    g_fprintf(outf, plural("Client check: %d host checked in %s seconds.", 
			 "Client check: %d hosts checked in %s seconds.",
			 hostcount),
	    hostcount, walltime_str(curclock()));
    g_fprintf(outf, plural("  %d problem found.\n",
			 "  %d problems found.\n", remote_errors),
	    remote_errors);
    fflush(outf);
    ...
    security_handle_t *	sech)
{
	...
	if(strncmp_const(line, "OPTIONS ") == 0) {
		...
		if((hostp->features = am_string_to_feature(t)) == NULL) {
		    g_fprintf(outf, "ERROR: %s: bad features value: '%s'\n",
			    hostp->hostname, t);
		    g_fprintf(outf, "The amfeature in the reply packet is invalid\n");
		    remote_errors++;
		    hostp->up = HOST_DONE;
		}
		if (u)
	...
	g_fprintf(outf, "ERROR: %s: unknown response: %s\n",
		hostp->hostname, line);
	remote_errors++;
	hostp->up = HOST_DONE;
    }
