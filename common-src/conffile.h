 ...
 *    config_name.
 *  - otherwise, if CONFIG_USE_CWD is set, use the current directory as 
 *    config_dir, and its filename as config_name.
...
/* Dump the current configuration information to filename, in a format 
 * that can be re-read by this module.  The results will include any
 * command-line overwrites.
 *
 * This function always dumps the server configuration.
 *
 * @param filename: filename to write to
 */
void dump_configuration(char *filename);

/* Return a printable representation of the given val_t.  If str_needs_quotes
 * is provided, a CONFTYPE_STR value will be returned with quotes.  Caller is
 ...
 */
char *val_t_display_str(val_t *val, int str_needs_quotes);
