...
static void validate_taperstart(conf_var_t *, val_t *);
...
/* Utility functions/structs for val_t_display_str */
...
conf_var_t server_var [] = {
   ...
   { CONF_TAPERFLUSH           , CONFTYPE_BOOLEAN  , read_bool        , CNF_TAPERFLUSH           , NULL },
...
validate_taperstart(
    ...
    val_t        *val)
{
    if(val_t__intrange(val)[0] < 0|| val_t__intrange(val)[0] > 100) {
       conf_parserror("taperstart value must be between 0 and 100");
    } else if(val_t__intrange(val)[1] < 0 || val_t__intrange(val)[1] > 100) {
       conf_parserror("taperstart value must be between 0 and 100");
    } else if(val_t__intrange(val)[0] > val_t__intrange(val)[1]) {
...
{
    assert(ttyp != NULL);
    assert(k >= 0 && k < TAPETYPE_TAPETYPE);
...
{
    assert(dtyp != NULL);
    assert(k >= 0 && k < DUMPTYPE_DUMPTYPE);
    ...
    char *filename)
{
	...
	for(i=0; i < HOLDING_HOLDING; i++) {
	    ...
	    msg = val_t_display_str(&hp->value[i], 1);
	    /* Print the keyword for anything that is not itself an identifier */
	    if (kt->token != CONF_IDENT)
		g_printf("      %-9s ", kt->keyword);
	    g_printf("%s\n", msg);
	    amfree(msg);
	}
	g_printf("}\n");
	...
	for(i=0; i < TAPETYPE_TAPETYPE; i++) {
	    ...
	    msg = val_t_display_str(&tp->value[i], 1);
	    /* Print the keyword for anything that is not itself an identifier */
	    if (kt->token != CONF_IDENT)
		g_printf("      %-9s ", kt->keyword);
	    g_printf("%s\n", msg);
	...
	if (strncmp_const(dp->name, "custom(") != 0) { /* don't dump disklist-derived dumptypes */
		...
		msg = val_t_display_str(&dp->value[i], 1);
		/* Print the keyword for anything that is not itself an identifier */
		if (kt->token != CONF_IDENT)
		    g_printf("%s      %-19s ", prefix, kt->keyword);
		g_printf("%s\n", msg);
		amfree(msg);
	    }
	    g_printf("%s}\n", prefix);
	...
	for(i=0; i < INTER_INTER; i++) {
	    ...
	    msg = val_t_display_str(&ip->value[i], 1);
	    /* Print the keyword for anything that is not itself an identifier */
	    if (kt->token != CONF_IDENT)
		g_printf("%s      %-19s ", prefix, kt->keyword);
	    g_printf("%s\n", msg);
    ...
    int    str_need_quote)
{
    ...
    switch(val->type) {
    case CONFTYPE_INT:
	buf = vstrallocf("%d", val_t__int(val));
	break;

    case CONFTYPE_SIZE:
	buf = vstrallocf("%zd", (ssize_t)val_t__size(val));
	break;

    case CONFTYPE_AM64:
	buf = vstrallocf("%lld", (long long)val_t__am64(val));
	break;

    case CONFTYPE_REAL:
	buf = vstrallocf("%0.5f", val_t__real(val));
	break;

    case CONFTYPE_RATE:
	buf = vstrallocf("%0.5f %0.5f", val_t__rate(val)[0], val_t__rate(val)[1]);
	break;

    case CONFTYPE_INTRANGE:
	buf = vstrallocf("%d,%d", val_t__intrange(val)[0], val_t__intrange(val)[1]);
	break;

    case CONFTYPE_IDENT:
	if(val->v.s) {
	    buf = stralloc(val->v.s);
        } else {
	    buf = stralloc("");
	}
	break;
    ...
    case CONFTYPE_TIME:
	buf = vstrallocf("%2d%02d",
			 (int)val_t__time(val)/100, (int)val_t__time(val) % 100);
	break;

    case CONFTYPE_EXINCLUDE: {
        char * buf1 = exinclude_display_str(val, 0);
        char * buf2 = exinclude_display_str(val, 1);
        buf = g_strconcat(buf1, "; ", buf2, NULL);
        amfree(buf1);
        amfree(buf2);
	break;
    }
	...
	case DS_STANDARD:
	    buf = vstrallocf("STANDARD");
	    break;

	case DS_NOFULL:
	    buf = vstrallocf("NOFULL");
	    break;

	case DS_NOINC:
	    buf = vstrallocf("NOINC");
	    break;

	case DS_HANOI:
	    buf = vstrallocf("HANOI");
	    break;

	case DS_INCRONLY:
	    buf = vstrallocf("INCRONLY");
	    break;
	}
	break;
	...
	case COMP_FAST:
	    buf = vstrallocf("CLIENT FAST");
	    break;

	case COMP_BEST:
	    buf = vstrallocf("CLIENT BEST");
	    break;

	case COMP_CUST:
	    buf = vstrallocf("CLIENT CUSTOM");
	    break;

	case COMP_SERVER_FAST:
	    buf = vstrallocf("SERVER FAST");
	    break;

	case COMP_SERVER_BEST:
	    buf = vstrallocf("SERVER BEST");
	    break;

	case COMP_SERVER_CUST:
	    buf = vstrallocf("SERVER CUSTOM");
	    break;
	}
	break;
	...
	case ES_CLIENT:
	    buf = vstrallocf("CLIENT");
	    break;

	case ES_SERVER:
	    buf = vstrallocf("SERVER");
	    break;

	case ES_CALCSIZE:
	    buf = vstrallocf("CALCSIZE");
	    break;
	}
	break;
	...
	case HOLD_REQUIRED:
	    buf = vstrallocf("REQUIRED");
	    break;
	}
	break;

     case CONFTYPE_TAPERALGO:
	buf = vstrallocf("%s", taperalgo2str(val_t__taperalgo(val)));
	break;
    ...
    case CONFTYPE_PROPLIST:
	buf = stralloc(""); /* the foreach_fn appends to this */
	g_hash_table_foreach(val_t__proplist(val), proplist_display_str_foreach_fn, &buf);
        break;
    }
    return buf;
}
    ...
    gpointer user_data_p)
{
    ...
    vstrextend(msg, "\"", property_s, "\" \"", value_s, "\"", NULL);
};
    ...
    int    file)
{
	...
	for(excl = sl->first; excl != NULL; excl = excl->next) {
            rval = newvstralloc(rval, " \"", excl->name, "\"", NULL);
	}
    }
