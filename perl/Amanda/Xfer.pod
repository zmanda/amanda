...
be finished.  Note that this element does not apply any sort of stream
buffering.
...
This class splits a data stream into parts on the storage media.  Since parts
are not cached, it cannot re-write a partial part, so if it is used with
devices that do not support LEOM, then it may fail an entire transfer if the
device reaches EOM.
...
a variety of ways, and supports "rewinding" to retry a failed part (e.g., one
that does not fit on a device).  It assumes that when a device reaches EOM
while writing, the entire file is corrupt - that is, that the device does not
support logical EOM.
