...
This method must be called in C<ACCESS_NULL> (that is, before C<start> or after
C<finish>).  Between the time C<listen> is called and when C<accept> returns, no
other methods of the device should be called.
...
connection ever occurs.

=head3 can_use_connection

  die "no-can-do!"
    unless $dev->can_use_connection($conn);

Not all DirectTCPConnections are created alike.  This method returns TRUE if
the given connection is compatible with this device.

=head3 write_from_connection

  ($ok, $actual_size) = $dev->write_from_connection($conn, $size);

This method reads data from an existing DirectTCPConnection (perhaps created
by another device) and writes it to the volume.   It writes at most C<$size>
bytes, and returns the number of bytes written in C<$actual_size>.  On error,
C<$ok> is false.
...
=head3 read_to_connection
  ...
  ($ok, $actual_size) = $dev->read_to_connection($conn, $size);
