...
have its default value.  C<getconf_linenum($cnf)> return the line number in the configuration file where it is set, 0 if it is not in a configuration file, -2 if it is set in a command line argument.
