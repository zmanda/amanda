...
got_dumps([ sortdumps_ref $catalog->get_dumps(parts => 1) ],
    [ sortdumps dumps_named qr/.*/ ],
    "get_dumps returns all dumps when given no parameters");

got_dumps([ sortdumps_ref $catalog->get_dumps(write_timestamp => '20080111000000', parts => 1) ],
    [ sortdumps dumps_named qr/somebox_lib_20080111/ ],
    "get_dumps parameter write_timestamp");

got_dumps([ sortdumps_ref $catalog->get_dumps(write_timestamp => '20080111', parts => 1) ],
    [ sortdumps dumps_named qr/somebox_lib_20080111/ ],
    "get_dumps accepts a short write_timestamp and zero-pads it");

got_dumps([ sortdumps_ref $catalog->get_dumps(write_timestamps => ['20080111000000','20080222222222'], parts => 1) ],
    [ sortdumps dumps_named qr/(20080111|20080222222222)$/ ],
    "get_dumps parameter write_timestamps");

got_dumps([ sortdumps_ref $catalog->get_dumps(hostname => 'otherbox', parts => 1) ],
    [ sortdumps dumps_named qr/^otherbox/ ],
    "get_dumps parameter hostname otherbox");

got_dumps([ sortdumps_ref $catalog->get_dumps(hostname => 'oldbox', parts => 1) ],
    [ sortdumps dumps_named qr/^oldbox_.*_holding/ ],
    "get_dumps parameter hostname oldbox, holding");

got_dumps([ sortdumps_ref $catalog->get_dumps(hostnames => ['notthere', 'otherbox'], parts => 1) ],
    [ sortdumps dumps_named qr/^otherbox/ ],
    "get_dumps parameter hostnames notthere|notthere");

got_dumps([ sortdumps_ref $catalog->get_dumps(hostname_match => 'other*', parts => 1) ],
    [ sortdumps dumps_named qr/^otherbox/ ],
    "get_dumps parameter hostname_match other*");

got_dumps([ sortdumps_ref $catalog->get_dumps(diskname => '/lib', parts => 1) ],
    [ sortdumps dumps_named qr/^[^_]*_lib_/ ],
    "get_dumps parameter diskname");

got_dumps([ sortdumps_ref $catalog->get_dumps(disknames => ['/lib', '/usr/bin'], parts => 1) ],
    [ sortdumps dumps_named qr/^[^_]*_(usr_bin|lib)_/ ],
    "get_dumps parameter disknames");

got_dumps([ sortdumps_ref $catalog->get_dumps(diskname_match => 'bin', parts => 1) ],
    [ sortdumps dumps_named qr/.*_bin_/ ],
    "get_dumps parameter diskname_match");

got_dumps([ sortdumps_ref $catalog->get_dumps(dump_timestamp => '20080414144444', parts => 1) ],
...
got_dumps([ sortdumps_ref $catalog->get_dumps(dump_timestamp_match => '200804-7', parts => 1) ],
    [ sortdumps dumps_matching { $_->{'dump_timestamp'} =~ /^20080[4567]/ } ],
    "get_dumps parameter dump_timestamp_match");

got_dumps([ sortdumps_ref $catalog->get_dumps(level => 0, parts => 1) ],
    [ sortdumps dumps_matching { $_->{'level'} == 0 } ],
    "get_dumps parameter level");

got_dumps([ sortdumps_ref $catalog->get_dumps(levels => [ 1 ], parts => 1) ],
    [ sortdumps dumps_matching { $_->{'level'} == 1 } ],
    "get_dumps parameter levels");

got_dumps([ sortdumps_ref $catalog->get_dumps(status => "OK", parts => 1) ],
    [ sortdumps dumps_matching { $_->{'status'} eq "OK" } ],
    "get_dumps parameter status = OK");

got_dumps([ sortdumps_ref $catalog->get_dumps(status => "PARTIAL", parts => 1) ],
    [ sortdumps dumps_matching { $_->{'status'} eq "PARTIAL" } ],
    "get_dumps parameter status = PARTIAL");

got_dumps([ sortdumps_ref $catalog->get_dumps(status => "FAIL", parts => 1) ],
    [ sortdumps dumps_matching { $_->{'status'} eq "FAIL" } ],
    "get_dumps parameter status = FAIL");

@dumpspecs = Amanda::Cmdline::parse_dumpspecs([".*", "/lib"], 0);
got_dumps([ sortdumps_ref $catalog->get_dumps(dumpspecs => [ @dumpspecs ], parts => 1) ],
...
$catalog->compute_retention($time5);
$volumes = $catalog->find_volumes(no_bless => 1);
my @volumes = sort {$a->{config} cmp $b->{config} or
		    $a->{storage} cmp $b->{storage} or
		    $a->{pool} cmp $b->{pool} or
		    $a->{label} cmp $b->{label} } @{$volumes};
is_deeply(\@volumes,
	  [
#0
           {
...
$catalog->compute_retention($time6);
$volumes = $catalog->find_volumes(no_bless => 1);
@volumes = sort {$a->{config} cmp $b->{config} or
		 $a->{storage} cmp $b->{storage} or
		 $a->{pool} cmp $b->{pool} or
		 $a->{label} cmp $b->{label} } @{$volumes};
is_deeply(\@volumes,
...
$catalog->compute_retention($time7);
$volumes = $catalog->find_volumes(no_bless => 1);
@volumes = sort {$a->{config} cmp $b->{config} or
		 $a->{storage} cmp $b->{storage} or
		 $a->{pool} cmp $b->{pool} or
...
$catalog->compute_retention($time8);
$volumes = $catalog->find_volumes(no_bless => 1);
@volumes = sort {$a->{config} cmp $b->{config} or
		 $a->{storage} cmp $b->{storage} or
		 $a->{pool} cmp $b->{pool} or
...
$catalog->compute_retention($time9);
$volumes = $catalog->find_volumes(no_bless => 1);
@volumes = sort {$a->{config} cmp $b->{config} or
		 $a->{storage} cmp $b->{storage} or
		 $a->{pool} cmp $b->{pool} or
...
$catalog->compute_retention($time10);
$volumes = $catalog->find_volumes(no_bless => 1);
@volumes = sort {$a->{config} cmp $b->{config} or
		 $a->{storage} cmp $b->{storage} or
		 $a->{pool} cmp $b->{pool} or
...
$catalog->compute_retention($time1);
$volumes = $catalog->find_volumes(no_bless => 1);
@volumes = sort {$a->{config} cmp $b->{config} or
		 $a->{storage} cmp $b->{storage} or
		 $a->{pool} cmp $b->{pool} or
...
$catalog->compute_retention($time2);
$volumes = $catalog->find_volumes(no_bless => 1);
@volumes = sort {$a->{config} cmp $b->{config} or
		 $a->{storage} cmp $b->{storage} or
		 $a->{pool} cmp $b->{pool} or
...
$catalog->compute_retention($time3);
$volumes = $catalog->find_volumes(no_bless => 1);
@volumes = sort {$a->{config} cmp $b->{config} or
		 $a->{storage} cmp $b->{storage} or
		 $a->{pool} cmp $b->{pool} or
...
$catalog->compute_retention($time4);
$volumes = $catalog->find_volumes(no_bless => 1);
@volumes = sort {$a->{config} cmp $b->{config} or
		 $a->{storage} cmp $b->{storage} or
		 $a->{pool} cmp $b->{pool} or
