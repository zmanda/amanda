...
my $a = '{"FAILURE":{"XFER_DEST_HOLDING_SHM_WRITE_HEADER":"1"}}';
my $post_data = '';
if (rand(2) > 1) {
    $post_data = '{"FAILURE":{"XFER_DEST_HOLDING_SHM_WRITE_HEADER":"1"}}';
} else {
    $post_data = '{"FAILURE":{"XFER_DEST_HOLDING_SHM_WRITE_HEADER":"1","DISABLE_NETWORK_SHM":"1"}}';
}
...
is_deeply($reply->{'body'}->[0]->{'report'}->{'failure_summary'}, [
        "  localhost diskname2 lev 0  FAILED [Failed to write header to holding file '$Installcheck::TMP/holding/$timestamp/localhost.diskname2.0.1.tmp': No space left on device]",
