...
my $post_data = '';
if (rand(2) > 1) {
    $post_data = '{"FAILURE":{"XFER_DEST_HOLDING_CLOSE_CHUNK":"1"}}';
} else {
    $post_data = '{"FAILURE":{"XFER_DEST_HOLDING_CLOSE_CHUNK":"1","DISABLE_NETWORK_SHM":"1"}}';
}
...
is_deeply($reply->{'body'}->[0]->{'report'}->{'failure_summary'}, [
        "  localhost diskname2 lev 0  FAILED [Failed to close holding file '$Installcheck::TMP/holding/$timestamp/localhost.diskname2.0.1': No space left on device]",
        "  localhost diskname2 lev 0  FAILED [Failed to close holding file '$Installcheck::TMP/holding/$timestamp/localhost.diskname2.0.1': No space left on device]",
	...
	'localhost' => {
		'diskname2' => {
			$timestamp => {
				...
				'esize' => '1075200',
				'retry_level' => '-1',
				'message' => "dump failed: Failed to close holding file '$Installcheck::TMP/holding/$timestamp/localhost.diskname2.0.1': No space left on device",
				'chunk_time' => undef,
				'dsize' => '1075200',
				'status' => '12',
				'level' => '0',
				'dump_time' => undef,
				'error' => "Failed to close holding file '$Installcheck::TMP/holding/$timestamp/localhost.diskname2.0.1': No space left on device",
...
-------------------------- ---------------------- ---------------- ---------------
localhost    diskname2   0    1050    1050    --     0:00 999999.9   0:00 999999.9 PARTIAL
...
localhost:diskname2 $timestamp 0      1050k dump failed: Failed to close holding file '$Installcheck::TMP/holding/$timestamp/localhost.diskname2.0.1': No space left on device, written (00:00:00)
...
taped           :   1      1050k      1050k (100.00%) (100.00%)
    tape 1      :   1      1050k      1050k (  3.23%) TESTCONF01 (1 parts)
