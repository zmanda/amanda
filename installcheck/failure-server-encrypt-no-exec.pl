...
localhost diskname2 $diskname {
    ...
    encrypt server
    server-encrypt "$cwd/amcat-error-no-exec"
}
EODLE
$testconf->write();
	...
	'localhost' => {
		'diskname2' => {
			$timestamp => {
				...
				'esize' => '1075200',
				'retry_level' => '-1',
				'message' => "dump failed: [write to encryption program failed: Broken pipe]",
				'chunk_time' => undef,
				'dsize' => '0',
				'status' => '12',
				'level' => '0',
				'dump_time' => undef,
				'error' => "[write to encryption program failed: Broken pipe]",
...
localhost:diskname2 $timestamp 0         0k dump failed: [write to encryption program failed: Broken pipe] (00:00:00)
