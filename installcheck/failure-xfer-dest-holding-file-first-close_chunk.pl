...
localhost diskname2 $diskname {
    ...
    server-custom-compress "$cwd/amcat"
}
EODLE
$testconf->write();
...
my $post_data = '';
if (rand(2) > 1) {
    $post_data = '{"FAILURE":{"XFER_DEST_HOLDING_CLOSE_CHUNK":"0"}}';
} else {
    $post_data = '{"FAILURE":{"XFER_DEST_HOLDING_CLOSE_CHUNK":"0","DISABLE_NETWORK_SHM":"1"}}';
}
...
is($reply->{'body'}->[0]->{'report'}->{'notes'}->[3], '  taper: tape TESTCONF01 kb 992 fm 1 [OK]' , 'notes[3] is correct');
...
is($reply->{'body'}->[0]->{'report'}->{'usage_by_tape'}->[0]->{'size'}, '992' , 'size 992  on tape 0');
...
is_deeply($reply->{'body'}->[0]->{'report'}->{'statistic'}->{'tape_size'}, { 'full' => '992',
									     'total' => '992',
...
is($reply->{'body'}->[0]->{'report'}->{'summary'}->[0]->{'dump_out_kb'}, '992', 'dump_out_kb is correct');
	...
	'localhost' => {
		'diskname2' => {
			$timestamp => {
				...
				'esize' => '537600',
				'retry_level' => '-1',
				'message' => "dump failed: Failed to close holding file '$Installcheck::TMP/holding/$timestamp/localhost.diskname2.0': No space left on device",
				'chunk_time' => undef,
				'dsize' => '1075200',
				'status' => '12',
				'level' => '0',
				'dump_time' => undef,
				'error' => "Failed to close holding file '$Installcheck::TMP/holding/$timestamp/localhost.diskname2.0': No space left on device",
...
NOTES:
  ...
  taper: tape TESTCONF01 kb 992 fm 1 [OK]
...
-------------------------- ---------------------- ---------------- ---------------
localhost    diskname2   0    1050     992    --     0:00 999999.9   0:00 999999.9 PARTIAL
...
localhost:diskname2 $timestamp 0      1050k dump failed: Failed to close holding file '$Installcheck::TMP/holding/$timestamp/localhost.diskname2.0': No space left on device, written (00:00:00)
...
dumping to tape
failed to tape
taped           :   1      1050k       525k (100.00%) (100.00%)
    tape 1      :   1       992k       992k (  3.23%) TESTCONF01 (1 parts)
