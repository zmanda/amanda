...
dvdrw_device_open_device(Device *dself, char *device_name, char *device_type, char *device_node)
{
	...
	device_set_error(dself,
	    g_strdup("DVDRW device requires cache directory and DVD-RW device separated by a colon (:) in tapedev"),
...
check_access_mode(DvdRwDevice *self, DeviceAccessMode mode)
{
    ...
    device_set_error(dself,
	g_strdup("DVDRW device can only be opened in READ or WRITE mode"),
	DEVICE_STATUS_DEVICE_ERROR);
...
check_readable(DvdRwDevice *self)
{
	...
	device_set_error(dself,
	    g_strdup("DVDRW device requires DVDRW_MOUNT_POINT to open device for reading"),
	    DEVICE_STATUS_DEVICE_ERROR);
...
execute_command(DvdRwDevice *self, gchar **argv, gint *result)
{
    ...
    if (!success) {
	gchar *error_message = g_strdup_printf("DVDRW device cannot execute '%s': %s (status: %d) (stderr: %s)",
	    argv[0], error ? error->message : "Unknown error", errnum, std_error ? std_error: "No stderr");
