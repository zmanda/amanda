...
sys.path.insert(0, '/opt/zmanda/amanda/python/lib64/python3.6/site-packages')
import rpmfile
...
def handle_argv_opts():
    ...
    if len(sys.argv) < 2:
        print("Usage: [ --clr-symlinks ] [ --stub-only ] /opt/zmanda/amanda/pkg-manifests/<manifest-file>")
        sys.exit(-1)
    ...
    opt_canOutput = sys.stdin.isatty() or sys.stdout.isatty()  # what is this????
...
class ManifestStub:
    ...
    def __init__(self, fstr):
        ...
        # realpath(path) gives the path eliminating any symbolic links

        if str(self._manifpath.parent) != '/opt/zmanda/amanda/pkg-manifests':
            # '.parent' gives the "cd .." of the specified path
        ...
        for fname in self._files:
            fd.write(str(fname, "utf8"))
        fd.close()
        self._dirs = self._rpm.headers.get('dirindexes')  # stores the index of the dirs corresponding to each file
        self._dirindex = self._rpm.headers.get('dirnames')  # stores the dir names
        ...
        # if self._rpm.headers.get('filedigests') returns nothing then "self._rpm.headers.get('filemd5s')"
        # is used to get the file hash code

        self.nfiles = len(self._files)
        # why have these been commented??
    ...
    @property
    def digestsSuffix(self):
        return self._digestsSuffix
...
class ManifestFile:
    ...
    def verify_string(self, st):
        ...
        return "{size}{mode}{digests}.{link}{uid}{gid}{mtime}.".format(
            ...
            mode=(
                'M' if (not self.issym() and self.isinstalled() and self.fmode() != stat.S_IMODE(st.st_mode)) else '.'),
    ...
    def do_sha256_check(self):  # sha256sum -c "file_name" : tells if it matches
        if not (self.isreg() and self.isinstalled()):
            return True
        with open("/tmp/sha.tmp", "w") as dig:
            tmp_str = self._manif._digests[self._n] + " " + self.fpath_bstr()
            dig.write(str)
            dig.close()
        val = not subprocess.run(["/usr/bin/sha256sum", "--quiet", "-c", "/tmp/sha.tmp"], check=False).returncode
        os.remove("/tmp/sha.tmp")
        return val

    # added
    def do_md5_check(self):  # md5sum -c "<file_name>" : tells if it matches
        if not (self.isreg() and self.isinstalled()):
            return True
        with open("/tmp/md5.tmp", "w") as dig:
            tmp_str = self._manif._digests[self._n] + " " + self.fpath_bstr()
            dig.write(str)
            dig.close()
        val = not subprocess.run(["/usr/bin/md5sum", "--quiet", "-c", "/tmp/md5.tmp"], check=False).returncode
        os.remove("/tmp/md5.tmp")
        return val
    ...
    def do_utime(self, atime):
        os.utime(self.my_fpath_bstr(), (atime, self._manif._mtimes[self._n]),
                 dir_fd=self._dirfd)  # already ignores symlinks
    ...
    for n in range(0, manif.nfiles):
        ...
        try:
            if not f.get_lstat_match():
                # will be satisfied if the file path does not exist or if the file is platform specific
                if not os.path.exists(f.fpath()):
                    continue
                os.rename(f.fpath(), "{}.{}.cfgsave".format(f.fpath(), os.getpid()))
                continue
            if not f.issym():  # satisfies if the path is not a symlink
                continue
            f.do_unlink()  # this happens only if the file is not a symlink
        except Exception as e:
            print("ERROR: failed to unlink: {} with {}".format(f.fpath(), str(e)))
            raise e

# added
# checking sha256sum of each file
if manif.digestsSuffix == ".sha256sums":
    file_list = []
    for n in range(0, manif.nfiles):
        f = ManifestFile(manif, n)
        try:
            if not f.do_sha256_check():
                continue
            else:
                file_list.append(f.fpath())
        except Exception as e:
            print("ERROR: failed to check sha256sum of {} due to {}".format(f.fpath(), str(e)))
            raise e
    if len(file_list) != 0:
        print("sha256sum check failed for the following files:\n {}".format(json.dumps(file_list)))

# checking md5sum of each file
if manif.digestsSuffix == ".md5sums":
    file_list = []
    for n in range(0, manif.nfiles):
        f = ManifestFile(manif, n)
        try:
            if not f.do_md5_check():
                continue
            else:
                file_list.append(f.fpath())
        except Exception as e:
            print("ERROR: failed to check md5sum of {} due to {}".format(f.fpath(), str(e)))
            raise e
    if len(file_list) != 0:
        print("md5sum check failed for the following files:\n {}".format(json.dumps(file_list)))
# added
...
for n in range(0, manif.nfiles):
    ...
    try:
        ...
        savcnf = os.path.isfile(nm + ".cfgsave")  # is it looking for a file similar to .rpmsave ??
        if opt_confSave:
            if not oldcnf:
                continue
            if savcnf:
                os.unlink(nm + ".cfgsave")  # what is a .cfgsave file?
...
try:
    ...
    i = l.rfind(".") + 1
    l = l[0:i] + "list"  # file list [this is adding the extension to name of
    # the file which has been extracted in line 361]
...
for n in range(0, manif.nfiles):
    ...
    if curdir != f.fdir_idx():
        try:
            ...
            print("{0:<9} {1}".format("d-error", curdirNm)) \
    ...
    try:
        ...
        if os.geteuid() == 0 and f.issym():
            f.do_symlink()
            print("{:<9} {} -> {}".format("new-sym", nm, f.lpath())) \
                if f.isinstalled() and opt_canOutput else 0
        ...
        if os.geteuid() == 0 and f.issym():
            f.do_symlink()
            print("{:<9} {} -> {}".format("io-sym", nm, f.lpath())) \
                if f.isinstalled() and opt_canOutput else 0
        else:
            print("{0:<9} {1}".format("error", nm)) \
                if f.isinstalled() and opt_canOutput else 0
        continue
    ...
    try:
        if os.geteuid() == 0:
            if (vfy.find('U') > 0 or vfy.find('G') > 0):  # isn't this script only for verifying? why are we
                # changing ownerships?? same goes for the following changes
                f.do_chown()

            if vfy.find('L') > 0:
                f.do_symlink()
                print("{:<9} {} -> {}".format("fix-sym", curdirNm + f.fname(), f.lpath())) \
                    if f.isinstalled() and opt_canOutput else 0
        ...
        print("{0} {1}".format("failed", nm)) if opt_canOutput else 0
