    ...
    application_argument_t *argument)
{
	...
	argv_ptr = amstar_build_argv(star_realpath,
				     argument, level, CMD_ESTIMATE, NULL);
	...
	if (WIFSIGNALED(wait_status)) {
	    ...
	    errmsg = vstrallocf(_("%s got bad exit: see %s"), star_realpath, dbfn());
	}
    ...
    application_argument_t *argument)
{
    ...
    argv_ptr = amstar_build_argv(star_realpath,
				 argument, level, CMD_BACKUP, mesgstream);
